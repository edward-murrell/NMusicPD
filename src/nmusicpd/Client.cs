...
namespace MPD {
	...
	public class MPDClient
	{
		...
		public bool Waiting {
			get {
				return (blockCommands.Count > 0 && ((Command)blockCommands[0]).Sent);
			}
		}	
		// Set BufferSize to 
		...
		public void Search(SearchType type, string what, bool exactmatch, 
			...
			string Type = Enum.GetName(typeof(SearchType), type).ToLower();
			string Command = "";
		...
		public bool ReadResponse() {
			...
			try {
				responses = (string[])
					Response.ReadResult(ServerNetworkStream, type) .ToArray(typeof(string));
